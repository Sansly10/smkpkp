<script>
    ...
    // Variabel gambar untuk ikon-ikon dalam tabel
    ...
    async function fetchUsers() {
      try {
        const response = await fetch('https://smkpkp.criwis-sjs.biz.id/api/admin_users');
        ...
        <tbody>
          {#each users as user}
            <tr>
              ...
              <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{user.username}</td>
